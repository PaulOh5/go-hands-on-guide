...
func TestCreateRepo(t *testing.T) {
	...
	r := svc.RepoCreateRequest{Body: &c}
	...
	for {
		...
		bData := svc.RepoCreateRequest_Data{Data: []byte{b}}
		r := svc.RepoCreateRequest{Body: &bData}
		err = stream.Send(&r)
		if err != nil {
			t.Fatal("StreamSend", err)
		}
	...
	if resp.Size != expectedSize {
		t.Errorf(
			"Expected Repo Craeted to be: %d bytes, Got back: %d",
